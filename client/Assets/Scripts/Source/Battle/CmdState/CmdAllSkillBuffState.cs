...
/// 全体BUFF
/// </summary>
public class CmdAllSkillBuffState : CmdStateBase
{
    ...
    public CmdAllSkillBuffState(BattleHero hero, IGUIBattle gui)
        : base(hero, gui)
    {
            ...
            for (int i = this.m_lstHitBool.Count - 1; i >= 0; i--)
            {
                ...
                if (dis < GAME_TIME.TIME_FIXED() - this.m_fAttackStartTime && !this.m_lstHitBool[i])
                {
                    ...
                    for (int j = 0; j < this.m_vecTargetHero.Length; j++)
                    {
                        ...
                        {
                            BATTLE_FUNCTION.SKILL_BUFF(this.m_cBattleHero, this.m_vecTargetHero[j], this.m_cGUIBattle);
                        }
                    }
