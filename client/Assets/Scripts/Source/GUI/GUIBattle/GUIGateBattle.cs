...
/// GUI关卡战斗
/// </summary>
public class GUIGateBattle : GUIBattle
{
    ...
    private GateTable m_cGateTable; //关卡表
    private DungeonTable m_cDungeonTable;   //副本表
    ...
    public override void Show()
    {
        ...
        this.m_cDungeonTable = WorldManager.GetDungeonTable(this.m_iDungeonID);
        this.m_cGateTable = WorldManager.GetGateTable(this.m_iGateID);
        ...
        //设置优惠类型
        this.m_eFavType = (FAV_TYPE)WorldManager.s_eCurDungeonFavType;
...
//        MediaMgr.PlayBGM(this.m_cGateTable.BGSound);
        ...
        {
            monsters[i] = MonsterTableManager.GetInstance().GetMonsterTable(this.m_iGateID, teamTable.VecMonster[i]);
        }
    ...
    private void BattleFinish()
    {
        ...
        DungeonTable dungeonTabel = WorldManager.GetDungeonTable(this.m_iDungeonID);

        HidenTopPanel();
        GUIBattleNext nextgui = (GUIBattleNext)this.m_cGUIMgr.GetGUI(GUI_DEFINE.GUIID_BATTLE_NEXT);
        nextgui.ShowLayer(this.m_iCurLayer + 1, this.m_cGateTable.MaxLayer, dungeonTabel.Name, this.m_cGateTable.Name, BattleNextFinish);
...
//            MediaMgr.PlayBGM(teamTable.BGSound);
    ...
    private void BattleEnd()
    {
        ...
        SendAgent.SendBattleGateFail(Role.role.GetBaseProperty().m_iPlayerId, Role.role.GetBaseProperty().m_iBattleID, vecItemTmp);
        Hiden();
    }
