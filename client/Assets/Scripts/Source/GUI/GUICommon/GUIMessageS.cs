...
/// GUI小消息框
/// </summary>
public class GUIMessageS: GUIBase
{
    private string RES_MAIN = "_GUI_MessageS";   //主资源
    ...
    private void OnPanel(GUI_INPUT_INFO info, object[] args)
    {
        ...
        {
            int temp = this.m_cGUIMgr.GetCurGUIID();

            Hiden();

            if (temp == GUI_DEFINE.GUIID_USERINFO)
            {
                GUIUserInfo infos = (GUIUserInfo)this.m_cGUIMgr.GetGUI(GUI_DEFINE.GUIID_USERINFO);
                infos.ShowPlayerInfo();
            }
        }
    }
}
