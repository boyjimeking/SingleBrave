...
public class GUIHeroUpgradeSelect : GUIHeroSelectBase
{
    ...
    protected override void InitGUI()
    {
		...
		List<Hero> partyHeros = allheros.FindAll(new Predicate<Hero>((item) => { return (item.m_iEquipID == -1) && item.m_iID != m_iBeUpgradeHeroID && !(CheckExistInTeams(HeroTeam.ToArray(), item.m_iID)) && (!item.m_bLock); }));
