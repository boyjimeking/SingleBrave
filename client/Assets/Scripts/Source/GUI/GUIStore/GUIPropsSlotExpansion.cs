...
public class GUIPropsSlotExpansion : GUIBase
{
    ...
    public GUIPropsSlotExpansion(GUIManager guiMgr)
        : base(guiMgr, GUI_DEFINE.GUIID_PROPSSLOTEXPANSION, GUILAYER.GUI_PANEL)
    { }
    ...
    protected override void InitGUI()
    {
        ...
        if (this.m_cGUIObject == null)
        {
            ...
            this.m_cLabHomeName.text = "物品栏扩张";
            ...
            recoverEvent.AddIntputDelegate(ExpansionPropsSlot);
            ...
            okEvent.AddIntputDelegate(ExpansionPropsSlot);
        ...
        sb.Append("花费" + GAME_DEFINE.DiamondPropsSlotCost + "个钻石可以将道具持有槽\n增加" + GAME_DEFINE.PropsSlotAddPerDiamond + "个\n\n#00FF00]现在的道具持有槽为");
        this.m_cLabelContent.text = sb.ToString() + Role.role.GetBaseProperty().m_iMaxItem + "个";
        ...
        gui.ChangeBottomLabel(GAME_FUNCTION.STRING(STRING_DEFINE.INFO_INCREASE_ITEM_MAX_NUMBER));
    }
    /// <summary>
    /// 隐藏
    /// </summary>
    ...
    /// <param name="info"></param>
    /// <param name="args"></param>
    private void ExpansionPropsSlot(GUI_INPUT_INFO info, object[] args)
    {
        if (info.m_eType == GUI_INPUT_INFO.GUI_INPUT_TYPE.CLICK)
        {
            if (!this.m_bIsSecondAlert)
            {
                ...
                {
                    //扩张道具槽
                    SendAgent.SendPropsSlotExpansionReq(Role.role.GetBaseProperty().m_iPlayerId);
                    this.m_bIsSecondAlert = false;
                }
                else
                {
