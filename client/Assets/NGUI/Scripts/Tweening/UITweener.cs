...
public abstract class UITweener : MonoBehaviour
{
    ...
    public delegate void FUNC2(object[] arg);
    [HideInInspector] public FUNC1 m_delFinish;  //回调一
    [HideInInspector] public FUNC2 m_delFinishArg;  //回调二
    [HideInInspector] public object[] m_vecArg2;  //回调参数2
	...
	void Update ()
	{
		...
		if ((style == Style.Once) && (mFactor > 1f || mFactor < 0f))
		{
            ...
            //有参回调
            if (this.m_delFinishArg != null)
                this.m_delFinishArg(this.m_vecArg2);
    ...
    /// <returns></returns>
    static public T Begin<T>(GameObject go, float delay , float duration) where T : UITweener
    {
        T comp = go.GetComponent<T>();
#if UNITY_FLASH
		if ((object)comp == null) comp = (T)go.AddComponent<T>();
        ...
        if (comp == null) comp = go.AddComponent<T>();
#endif
        comp.StartCoroutine(BeginDelay<T>(comp, delay, duration));
        return comp;
    }
    ...
    /// <param name="duration"></param>
    /// <returns></returns>
    private static IEnumerator BeginDelay<T>(T comp, float delay, float duration) where T : UITweener
    {
        comp.mStarted = false;
        comp.duration = duration;
        comp.mFactor = 0f;
